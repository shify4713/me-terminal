(() => {
  ...
  let usersList = [];
  let state = { items: [], craftables: [], cpus: [], power: {}, online: false, updated: null, error: null };
  ...
  function ensurePager() {
    ...
    pager.innerHTML = '<button type="button" class="btn secondary" id="pagePrev">←</button><span id="pageInfo">1 / 1</span><button type="button" class="btn secondary" id="pageNext">→</button><span id="pageTotal" class="pager-total"></span>';
  ...
  function tryLogin() {
    try {
      ...
      for (var i = 0; i < usersList.length; i++) {
        if (usersList[i].login === login && String(usersList[i].password) === String(pass)) { user = usersList[i]; break; }
      }
  ...
  function iconHtml(item) {
    ...
    if (item.icon) {
      return '<img class="item-icon" src="' + appBase() + "icons/" + item.icon + '" alt="" loading="lazy" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\'"/><div class="item-placeholder" style="display:none;background:' + modColor(item.mod) + '">' + name + "</div>";
    }
  ...
  function buildFiltered() {
    ...
    if (q) list = list.filter(function (i) {
      return (i.id || "").toLowerCase().indexOf(q) >= 0 || (i.label || "").toLowerCase().indexOf(q) >= 0 || (i.mod || "").toLowerCase().indexOf(q) >= 0;
    });
  ...
  function renderPage() {
    ...
    if (!state.items.length && !state.online) {
      ...
      if (els.emptyState) {
        ...
        els.emptyState.textContent = state.error ? ("Ошибка Gist: " + state.error) : (cfg.gistId ? "МЭ пуста или OC ещё не отправил данные. Запусти me_terminal.lua" : "Укажи Gist в ⚙ Settings (username = shify4713)");
      }
    } else if (!slice.length) {
      els.itemsGrid.innerHTML = "";
      if (els.emptyState) {
        els.emptyState.hidden = false;
        els.emptyState.textContent = total === 0 && (state.items || []).length ? "Ничего не найдено по фильтру" : "Нет предметов";
      }
    } else {
  ...
  function renderCraftables() {
    ...
    if (!list.length) {
      els.craftGrid.innerHTML = '<div class="empty-tab">Нет шаблонов крафта в МЭ.<br>OC пришлёт craftables, когда сеть онлайн.</div>';
      return;
    }
    var max = 96;
    var shown = list.slice(0, max);
    els.craftGrid.innerHTML = shown.map(function (i) { return renderItemCard(i, { showCraftDot: false }); }).join("") + (list.length > max ? '<div class="empty-tab">… и ещё ' + (list.length - max) + "</div>" : "");
  }
  function renderCpus() {
    if (!els.cpusList) return;
    var cpus = state.cpus || [];
    if (!cpus.length) { els.cpusList.innerHTML = '<div class="empty-tab">Нет crafting CPU в сети.</div>'; return; }
  ...
  function renderNetwork() {
    ...
    els.networkCards.innerHTML = cards.map(function (c) {
      return '<div class="net-card"><div class="net-card-label">' + c.label + '</div><div class="net-card-value">' + c.value + "</div></div>";
    }).join("");
  }
  function updateHeader() {
    ...
    if (els.connStatus) {
      els.connStatus.innerHTML = state.online ? '<span class="dot online"></span> Live' : '<span class="dot offline"></span> ' + (state.error ? "Error" : "Offline");
    }
  }
  ...
  function fullRender() { populateModFilter(); renderPage(); renderCraftables(); renderCpus(); renderNetwork(); updateHeader(); }
  ...
  function closeCraftModal() {
    if (els.craftModal) { els.craftModal.hidden = true; els.craftModal.style.display = "none"; }
  ...
  async function submitCraft() {
    ...
    try {
      ...
      try {
        ...
        if (r.ok) { queue = JSON.parse((await r.text()) || "[]"); if (!Array.isArray(queue)) queue = []; }
      } catch (e) {}
  ...
  async function loadFromGist() {
    var url = stateUrl();
    if (!url) { state.error = "Gist не настроен"; return false; }
    try {
  ...
  function closeSettings() {
    if (els.settingsModal) { els.settingsModal.hidden = true; els.settingsModal.style.display = "none"; }
  }
  ...
  function bindEvents() {
    ...
    document.addEventListener("click", function (e) {
      if (e.target && e.target.id === "pagePrev") { page = Math.max(0, page - 1); renderPage(); }
      if (e.target && e.target.id === "pageNext") { page = page + 1; renderPage(); }
    });
